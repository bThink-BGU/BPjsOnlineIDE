...
public class StepTest {
	...
	public void toStepMessage() {
		...
		try {			
			...
			assertEquals(stepMessage.getGlobalVars().size(), 4);
			assertEquals(stepMessage.getGlobalVals().size(), 4);
	...
	@Test //10.5
	public void toStepMessageWithEventNull() {
		StepMessage stepMessage = new StepMessage(null, null, null, null, null, null, null, null);
		Step step = getStep(stepMessage);
		...
		try {			
			...
			StepMessage newStepMessage = new StepMessage(stepMessage.getBpss(), new HashMap<>(), 
					stepMessage.getbThreads(), stepMessage.getReqList(), 
					stepMessage.getSelectableEvents(), stepMessage.getWaitList(), 
					stepMessage.getBlockList(), null);
			
			newStepMessage.setGlobalVars(stepMessage.getGlobalVars());
			newStepMessage.setGlobalVals(stepMessage.getGlobalVals());
			
			assertEquals(newStepMessage.getGlobalVars().size(), 4);
			assertEquals(newStepMessage.getGlobalVals().size(), 4);
			
			
			assertEquals(newStepMessage.getReqList().size(), 1);
			assertTrue(newStepMessage.getReqList().contains("[BEvent name:b]"));
			
			assertEquals(newStepMessage.getSelectableEvents().size(), 1);
			assertTrue(newStepMessage.getSelectableEvents().contains("[BEvent name:b]"));
			
			assertEquals(newStepMessage.getWaitList().size(), 1);
			assertTrue(newStepMessage.getWaitList().contains("{none}"));
			
			assertEquals(newStepMessage.getBlockList().size(), 1);
			assertTrue(newStepMessage.getBlockList().contains("{none}"));			
			
			assertEquals(newStepMessage.getSelectedEvent(), null);
	...
	private Step getStep(StepMessage stepMessage) {
		try {
